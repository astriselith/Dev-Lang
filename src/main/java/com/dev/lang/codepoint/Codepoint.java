package com.dev.lang.codepoint;
...
public final class Codepoint {
	...
	public static int width(byte b) {
		int v = b & 0xFF;
		if ((v & 0x80) == 0) return 1;
		if ((v & 0xE0) == 0xC0) return 2;
		if ((v & 0xF0) == 0xE0) return 3;
		if ((v & 0xF8) == 0xF0) return 4;
		return 0;
	}
	...
	public static boolean equalsIgnoreCase(int cp1, int cp2) {
		...
		if (cp1 >= 'A' && cp1 <= 'Z') return cp1 + 32 == cp2;
		if (cp1 >= 'a' && cp1 <= 'z') return cp1 - 32 == cp2;
		return false;
	}
	...
	public static String toString(int codepoint) {
		...
		String format = codepoint <= 0xFFFF ? "U+%04X ('%s')" : "U+%06X ('%s')";
		return String.format(format, codepoint, charStr);
	}
}